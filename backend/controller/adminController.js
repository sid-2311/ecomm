...
const registerAdmin = async (req, res) => {
  try {
    ...
    if (isAdded) {
      ...
      // Handle name field - Admin model expects name as Object
      // If name is a string, convert it to object format
      let nameObject = req.body.name;
      if (typeof req.body.name === "string") {
        nameObject = { en: req.body.name, de: req.body.name };
      } else if (!req.body.name || typeof req.body.name !== "object") {
        nameObject = { en: "Admin", de: "Admin" };
      }
...
const loginAdmin = async (req, res) => {
  try {
    ...
    if (admin && bcrypt.compareSync(req.body.password, admin.password)) {
      ...
      // Handle case where access_list might be undefined or null
      const accessList = Array.isArray(admin?.access_list) 
        ? admin.access_list 
        : [];
