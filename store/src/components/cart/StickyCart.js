  ...
  return (
    <button aria-label="Cart" onClick={toggleCartDrawer} className="absolute">
      <div className="right-0 w-35 float-right fixed top-2/4 bottom-2/4 align-middle shadow-lg cursor-pointer z-30 hidden lg:block xl:block">
        <div className="flex flex-col items-center justify-center bg-black rounded-tl-lg p-2 text-[#D9AA34] ">
          <span className="text-2xl mb-1 text-[#D9AA34] ">
            <IoBagHandleOutline />
          </span>
        ...
        <div className="flex flex-col items-center justify-center bg-[#D9AA34]  p-2 text-white text-base font-serif font-medium rounded-bl-lg mx-auto">
          {currency}
          {cartTotal.toFixed(2)}
        </div>
      </div>
